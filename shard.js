...
fastify.get("/invite", async (req, res) => {
    res.redirect("https://discord.com/oauth2/authorize?client_id=702201518329430117&permissions=2416307264&redirect_uri=https%3A%2F%2Fdiscord.gg%2FZr5eF5D&scope=bot%20applications.commands")
})
...
fastify.get("/v1/rso/redirect/:state", async (req, res) => {
    res.redirect(301, `https://auth.riotgames.com/login#client_id=valorantlabs&redirect_uri=https%3A%2F%2Fvalorantlabs.xyz%2Foauth-finished.html&response_type=code&scope=openid%20offline_access&prompt=login&state=${req.params.state}`)
})
...
fastify.get("/oauth-finished.html", async (req, res) => {
    if(req.query.state) {
        ...
        const tokens = await axios.post("https://auth.riotgames.com/token", formData, {headers: {"Authorization": `Basic ${Buffer.from(basedata.client_secret).toString("base64")}`}}).catch(error => {return error})
        const userinfo = await axios.get("https://europe.api.riotgames.com/riot/account/v1/accounts/me", {headers: {"Authorization": `Bearer ${tokens.data.access_token}`}}).catch(error => {return error})
        ...
        const region = await axios.get(`https://europe.api.riotgames.com/riot/account/v1/active-shards/by-game/val/by-puuid/${userinfo.data.puuid}`, {headers: {"X-Riot-Token": basedata.riottoken}}).catch(error => {return error})
        if(region.response) return res.code(500).send({error: `There seems to be an error with region of your account | Status: ${region.response.status} | Message: ${region.response.message}`})
    ...
    formData.append('redirect_uri', "https://valorantlabs.xyz/oauth-finished.html")
    var tokens = await axios.post("https://auth.riotgames.com/token", formData, {headers: {"Authorization": `Basic ${Buffer.from(`valorantlabs:4uxlkqJzyO5Dm1FOkp5yH8_1WW2d64TL-Xz54535XT0`).toString("base64")}`}}).catch(error => {return error})
    var userinfo = await axios.get("https://europe.api.riotgames.com/riot/account/v1/accounts/me", {headers: {"Authorization": `Bearer ${tokens.data.access_token}`}}).catch(error => {return error})
    ...
    if(dbentry == null) {
        ...
        var ingamepuuid
        var type
        var ingameapi = await axios.get(`https://api.henrikdev.xyz/valorant/v1/account/${encodeURI(userinfo.data.gameName)}/${encodeURI(userinfo.data.tagLine)}?asia=true`).catch(error => {return error})
        if(ingameapi.status) {region = ["eu", "ap", "na", "kr"].includes(ingameapi.data.data.region) ? ingameapi.data.data.region : null; ingamepuuid = ingameapi.data.data.puuid; type = ["eu", "ap", "na", "kr"].includes(ingameapi.data.data.region) ? "unofficial" : "official"}
        if(ingameapi.response) {region = null; ingamepuuid = null, type = "official"}
        var tracker = await axios.get(`https://api.henrikdev.xyz/valorant/v2/profile/${encodeURI(userinfo.data.gameName)}/${encodeURI(userinfo.data.tagLine)}`, {headers: {"z-api-token-trn": basedata.hdevtoken}}).catch(error => {return error})
        if(!tracker.response) {
            var agent_array = []
            var matches_array = []
            for(agent of tracker.data.agents) {
                agent_array.push({agent: agent.agent, playtime: agent.playtime.playtimeraw, matches: agent.matches, kills: agent.kills, deaths: agent.deaths, assists: agent.assists, headshots: agent.headshots, wins: agent.wins, firstbloods: agent.firstbloods, aces: agent.aces, clutches: agent.clutches, flawless: agent.flawless})
            }
            if(type == "official") region = await axios.get(`https://europe.api.riotgames.com/riot/account/v1/active-shards/by-game/val/by-puuid/${userinfo.data.puuid}`, {headers: {"X-Riot-Token": basedata.riottoken}}).catch(error => {return error})
                            ...
                            if(cmatch.data.data.metadata.mode != "Deathmatch") {
                                if(cmatch.data.data.metadata.mode != "Custom Game") {
                                    ...
                                    if(player != null) {
                                        ...
                                        if(dbcheck != undefined) {
                                            matches_array.push({won: team.has_won, gamekey: dbcheck.gamekey, id: matchid.matchId, start: cmatch.data.data.metadata.game_start * 1000, agent: player.character, mode: cmatch.data.data.metadata.mode, map: cmatch.data.data.metadata.map, teamblue_won: team.has_won, teamblue_rounds: team.rounds_won, teamred_won: team.has_won == true ? false : true, teamred_rounds: team.rounds_lost, kills: player.stats.kills, deaths: player.stats.deaths, assists: player.stats.assists})
                                        } else {
                                            ...
                                            while(!available) {
                                                ...
                                                if(ridcheck == undefined) {
                                                    ...
                                                    matches_array.push({won: team.has_won, gamekey: rid, id: matchid.matchId, start: cmatch.data.data.metadata.game_start * 1000, agent: player.character, mode: cmatch.data.data.metadata.mode, map: cmatch.data.data.metadata.map, teamblue_won: team.has_won, teamblue_rounds: team.rounds_won, teamred_won: team.has_won == true ? false : true, teamred_rounds: team.rounds_lost, kills: player.stats.kills, deaths: player.stats.deaths, assists: player.stats.assists})
                                                    available = true
                                                } else {
                                                    rid = randomize("Aa0", 6)
                                                }
                                ...
                                if(cmatch.data.data.metadata.mode != "Custom Game") {
                                    ...
                                    if(player != null) {
                                        ...
                                        const dbcheck = await getDB("games").findOne({matchid: matchid.matchId})
                                        if(dbcheck != undefined) {
                                            matches_array.push({won: team.puuid == ingamepuuid ? true : false, gamekey: dbcheck.gamekey, id: matchid.matchId, start: cmatch.data.data.metadata.game_start * 1000, agent: player.character, mode: cmatch.data.data.metadata.mode, map: cmatch.data.data.metadata.map, teamblue_won: team.puuid == ingamepuuid ? true : false, teamblue_rounds: team.puuid == ingamepuuid ? 1 : 0, teamred_won: team.puuid != ingamepuuid ? true : false, teamred_rounds: team.puuid != ingamepuuid ? 1 : 0, kills: player.stats.kills, deaths: player.stats.deaths, assists: player.stats.assists})
                                        } else {
                                            var rid = randomize("Aa0", 6)
                                            let available = false
                                            while(!available) {
                                                ...
                                                if(ridcheck == undefined) {
                                                    await getDB("games").insertOne({gamekey: rid, matchid: matchid.matchId})
                                                    matches_array.push({won: team.puuid == ingamepuuid ? true : false, gamekey: rid, id: matchid.matchId, start: cmatch.data.data.metadata.game_start * 1000, agent: player.character, mode: cmatch.data.data.metadata.mode, map: cmatch.data.data.metadata.map, teamblue_won: team.puuid == ingamepuuid ? true : false, teamblue_rounds: team.puuid == ingamepuuid ? 1 : 0, teamred_won: team.puuid != ingamepuuid ? true : false, teamred_rounds: team.puuid != ingamepuuid ? 1 : 0, kills: player.stats.kills, deaths: player.stats.deaths, assists: player.stats.assists})
                                                    available = true
                                                } else {
                                                    rid = randomize("Aa0", 6)
                                                }
                            ...
                            if(cmatch.data.matchInfo.queueId != "deathmatch") {
                                if(cmatch.data.matchInfo.queueId != "") {
                                    ...
                                    if(player != null) {
                                        ...
                                        const dbcheck = await getDB("games").findOne({matchid: matchid.matchId})
                                        if(dbcheck != undefined) {
                                            matches_array.push({won: team.won, gamekey: dbcheck.gamekey, id: matchid.matchId, start: cmatch.data.matchInfo.gameStartMillis, agent: agents[player.characterId], mode: gamemodes[cmatch.data.matchInfo.queueId], map: maps[cmatch.data.matchInfo.mapId], teamblue_won: team.won, teamblue_rounds: team.roundsWon, teamred_won: team2[0].won, teamred_rounds: team2[0].roundsWon, kills: player.stats.kills, deaths: player.stats.deaths, assists: player.stats.assists})
                                        } else {
                                            var rid = randomize("Aa0", 6)
                                            let available = false
                                            while(!available) {
                                                ...
                                                if(ridcheck == undefined) {
                                                    await getDB("games").insertOne({gamekey: rid, matchid: matchid.matchId})
                                                    matches_array.push({won: team.won, gamekey: rid, id: matchid.matchId, start: cmatch.data.matchInfo.gameStartMillis, agent: agents[player.characterId], mode: gamemodes[cmatch.data.matchInfo.queueId], map: maps[cmatch.data.matchInfo.mapId], teamblue_won: team.won, teamblue_rounds: team.roundsWon, teamred_won: team2[0].won, teamred_rounds: team2[0].roundsWon, kills: player.stats.kills, deaths: player.stats.deaths, assists: player.stats.assists})
                                                    available = true
                                                } else {
                                                    rid = randomize("Aa0", 6)
                                                }
            ...
            await getDB("userstats").updateOne({puuid: userinfo.data.puuid}, {$set: {puuid: userinfo.data.puuid, ingamepuuid: ingamepuuid, region: type == "unofficial" ? region : region.data.activeShard, type: type, tracker: true, last_update: Date.now(), stats: {playtime: tracker.data.stats.playtime.playtime_raw, matches: tracker.data.stats.matches, kills: tracker.data.stats.kills, deaths: tracker.data.stats.deaths, assists: tracker.data.stats.assists, headshots: tracker.data.stats.headshots, wins: tracker.data.stats.wins, firstbloods: tracker.data.stats.firstbloods, aces: tracker.data.stats.aces, clutches: tracker.data.stats.clutches, flawless: tracker.data.stats.flawless}, agents: agent_array, matches: matches_array}}, {upsert: true})
                            ...
                            if(cmatch.data.data.metadata.mode != "Deathmatch") {
                                if(cmatch.data.data.metadata.mode != "Custom Game") {
                                    ...
                                    if(player != undefined) {
                                        ...
                                        var agentarray = agent != undefined ? agent : {agent: "", playtime: 0, matches: 0, kills: 0, deaths: 0, assists: 0, headshots: 0, wins: 0, firstbloods: 0, aces: 0, clutches: 0, flawless: 0}
                                ...
                                if(cmatch.data.data.metadata.mode != "Custom Game") {
                                    ...
                                    if(player != undefined) {
                                        ...
                                        const dbcheck = await getDB("games").findOne({matchid: matchid.matchId})
                                        if(dbcheck != undefined) {
                                            matches_array.push({won: team.puuid == ingamepuuid ? true : false, gamekey: dbcheck.gamekey, id: matchid.matchId, start: cmatch.data.data.metadata.game_start, agent: player.character, mode: cmatch.data.data.metadata.mode, map: cmatch.data.data.metadata.map, teamblue_won: team.puuid == ingamepuuid ? true : false, teamblue_rounds: team.puuid == ingamepuuid ? 1 : 0, teamred_won: team.puuid != ingamepuuid ? true : false, teamred_rounds: team.puuid != ingamepuuid ? 1 : 0, kills: player.stats.kills, deaths: player.stats.deaths, assists: player.stats.assists})
                                        } else {
                                            var rid = randomize("Aa0", 6)
                                            let available = false
                                            while(!available) {
