...
export const errorhandler = async function (error) {
	if (error.status == 451) {
		...
		return error.message.reply({
			embeds: [
				embedBuilder({
					title: translations[error.lang].response[451].title,
					desc: translations[error.lang].response[451].description,
					...
					components: [
						{
							...
							label: translations[error.lang].response[451].component_login,
							style: ButtonStyle.Link,
							url: `https://valorantlabs.xyz/v1/rso/redirect/${uuid}`,
						},
						{
							type: ComponentType.Button,
							label: translations[error.lang].response[451].component_update,
							style: ButtonStyle.Danger,
							customId: `stats;update;${error.name};${error.tag}`,
						},
						{
							type: ComponentType.Button,
							label: translations[error.lang].response[451].component_rank,
							style: ButtonStyle.Danger,
							customId: `mmr;${error.name};${error.tag}`,
						},
					],
				},
			],
	...
	if (!translations[error.lang].response[error.status])
		return error.message.reply({
			embeds: [
				embedBuilder({
					title: translations[error.lang].response[500].title,
					desc: translations[error.lang].response[500][error.type],
	...
	if (!translations[error.lang].response[error.status][error.type])
		return error.message.reply({
			embeds: [
				embedBuilder({
					title: translations[error.lang].response[error.status].title,
					desc: translations[error.lang].response[error.status]['default'],
					footer: `VALORANT LABS [ERROR ${error.status}]`,
				}),
			],
			components: [
				{
	...
	return error.message.reply({
		embeds: [
			embedBuilder({
				title: translations[error.lang].response[error.status].title,
				desc: translations[error.lang].response[error.status][error.type],
				footer: `VALORANT LABS [ERROR ${error.status}]`,
			}),
		],
		components: [
...
export const errorhandlerinteraction = async function (error) {
	if (error.status == 451) {
		...
		return error.interaction.editReply({
			embeds: [
				embedBuilder({
					title: translations[error.lang].response[451].title,
					desc: translations[error.lang].response[451].description,
