    ...
    for(let i = 0; types.length > i; i++) {
        if(i == 0 || i == 1) {
            for(let k = 0; ccodes.length > k; k++) {
                const website = await axios.get(i == 0 ? translations[ccodes[k]].patchurl : translations[ccodes[k]].websiteurl).catch(error => {return error})
                if(!website.response) {
                    ...
                    if(article.length) {
                        getDB("websitecheck").updateOne({code: ccodes[k]}, {$set: i == 0 ? {patchnotes: moment(article[0].date).unix()} : {datewebsite: moment(article[0].date).unix()}})
                        let fetch = await getDB("settings").find({lang: ccodes[k]}, i == 0 ? {gid: 1, lang: 1, news: 1} : {gid: 1, lang: 1, onews: 1}).toArray()
                        //fetch = fetch.filter(item => item[nstatus[i]] !== false && item[nstatus[i]] !== "false")
                        article.forEach(article => {
                            fetch.forEach(guild => {
                                if(guild[nstatus[i]]) {
                                    manager.broadcastEval((client, {channelid, title, external_link, url, banner_url}) => {
                                        try {
                                    ...
                                    }, {context: {channelid: guild[nstatus[i]].replace(/[^0-9]/g, ''), title: article.title, external_link: article.external_link, url: article.url, banner_url: article.banner_url}})
                                }
                            })
                        })
                    }
        ...
        if(i == 2 || i == 3) {
            for(let k = 0; ccodes.length > k; k++) {
                const website = await axios.get(translations[ccodes[k]].statusurl).catch(error => {return error})
                if(!website.response) {
                    if(i == 2 ? website.data.data.maintenances.length : website.data.data.incidents.length) {
                        ...
                        const db = await getDB("websitecheck").findOne({code: ccodes[k]})
                        if(i == 2 ? moment(article.updates[0].created_at).unix() > db.datestatusmaintenance : moment(article.updates[0].created_at).unix() > db.datestatusincidents) {
                            i == 2 ? bot.db("VALORANT-LABS").collection("websitecheck").updateOne({code: ccodes[k]}, {$set: {datestatusmaintenance: moment(article.updates[0].created_at).unix()}}) : bot.db("VALORANT-LABS").collection("websitecheck").updateOne({code: ccodes[k]}, {$set: {datestatusincidents: moment(article.updates[0].created_at).unix()}})
                            const fetch = await getDB("settings").find({lang: ccodes[k]}, {gid: 1, lang: 1, serverstatus: 1}).toArray()
                            fetch.forEach(guild => {
                                if(guild.serverstatus) {
                                    manager.broadcastEval((client, {channelid, desc, t, create, platform, postedat, platforms}) => {
                                        try {
                                            if(client.channels.cache.has(channelid)) {
                                                client.channels.cache.get(channelid).send({
                                                    embeds: [{
                                        ...
                                        context: {
                                            ...
                                            desc: article.updates[0].translations.find(c => c.locale == websites[ccodes[k]].locale).content != undefined ? article.updates[0].translations.find(c => c.locale == websites[ccodes[k]].locale).content : article.updates[0].translations.find(c => c.locale == 'en_US').content,
                                            t: article.titles.find(c => c.locale == websites[ccodes[k]].locale).content != undefined ? article.titles.find(c => c.locale == websites[ccodes[k]].locale).content : article.titles.find(c => c.locale == 'en_US').content,
...
export const shard_status_update = async function (manager) {
    ...
    for(let i = 0; sharddata.length > i; i++) {
        fields.push({name: `Shard ${i}`, value: `Status: ${shard_status_codes[sharddata[i].status]} | Ping: ${sharddata[i].ping} | Memory: ${pretty(sharddata[i].mem, {locale: 'en'})}`})
    }
...
export const guildSettings = async function (guild) {
    return (await getDB("settings").findOneAndUpdate({gid: guild.id}, {$setOnInsert: {news: false, onews: false, serverstatus: false, lang: locales[guild.preferredLocale] ? locales[guild.preferredLocale] : "en-us", blacklist: false, prefix: "v?", background_stats: false, background_game: false, background_mmr: false, autoroles: []}}, {upsert: true, returnDocument: "after"})).value
}
...
export const buildStatsImage = async function ({dbstats, agent, modes, bgcanvas} = {}) {
    ...
    const rank = dbstats.ingamepuuid ? await axios.get(`https://api.henrikdev.xyz/valorant/v1/by-puuid/mmr/${dbstats.region}/${dbstats.ingamepuuid}`).catch(error => {return error}) : null
    let rank_image
    if(rank == null || rank.response || (rank.data && rank.data.data.currenttier == null)) {
        ...
        rank_image = await Canvas.loadImage(`https://media.valorant-api.com/competitivetiers/e4e9a692-288f-63ca-7835-16fbf6234fda/${rank.data.data.currenttier}/largeicon.png`)
        buildText({ctx: ctx, text: rank.data.data.mmr_change_to_last_game > 0 ? `+${rank.data.data.mmr_change_to_last_game}` : rank.data.data.mmr_change_to_last_game, size: 50, x: 1200, y: 820})
    }
    ...
    buildText({ctx: ctx, text: `${moment.duration(est).days()}D ${moment.duration(est).hours()}H ${moment.duration(est).minutes()}M ${moment.duration(est).seconds()}S`, size: 80, x: 1375, y: 1150, color: "#00ffff", align: "left"})
    ...
    if(best_agent) {
        if(!agent.response) {
            ...
            buildText({ctx: ctx, text: `${moment.duration(best_agent.playtime).days()}D ${moment.duration(best_agent.playtime).hours()}H ${moment.duration(best_agent.playtime).minutes()}M ${moment.duration(best_agent.playtime).seconds()}S`, size: 80, x: 3125, y: 1150, color: "#00ffff"})
        }
    }
...
export const patchStats = async function ({dbstats, mmatches, message, lang, agent, modes, bgcanvas} = {}) {
    ...
    if(message) {
        ...
        message.edit({embeds: [], files: [attachment], components: [{type: ComponentType.ActionRow, components: [{type: ComponentType.SelectMenu, customId: `game`, maxValues: 1, minValues: 0, options: [...new Set(components)], placeholder: translations[lang].stats.game_select}]}]})
    }
}
export const buildGameImage = async function ({id, guilddata, matchid, bgcanvas} = {}) {
    ...
    const match = await axios.get(`https://api.henrikdev.xyz/valorant/v2/match/${matchid ? matchid : gamekey.matchid}`).catch(error => {return error})
    ...
    switch(match.data.data.metadata.mode) {
        case "Deathmatch":
            ...
            for(let i = 0; sorted_array.length > i; i++) {
                ...
                fields.push({name: `${sorted_array[i].name}#${sorted_array[i].tag}`, value: `${(rank.response != null || rank.data.data.currenttier == null) ? ranks[0].discordid : ranks[rank.data.data.currenttier].discordid} | Score: ${sorted_array[i].stats.score} | KDA: ${sorted_array[i].stats.kills}/${sorted_array[i].stats.deaths}/${sorted_array[i].stats.assists}`})
            }
            return {
                error: null,
                unknown: null,
        ...
        default:
            ...
            buildText({ctx: ctx, text: `${match.data.data.metadata.mode} | ${match.data.data.metadata.map} | ${match.data.data.metadata.cluster} | ${match.data.data.metadata.game_version.split("-")[0]} -${match.data.data.metadata.game_version.split("-")[1]}`, size: 100, x: 950, y: 1075})
            buildText({ctx: ctx, text: `${moment.duration(match.data.data.metadata.game_length).minutes()}m ${moment.duration(match.data.data.metadata.game_length).seconds()}s | ${moment(match.data.data.metadata.game_start * 1000).format("LLLL")}`, size: 100, x: 950, y: 1200, color: "#ffffff"})
            ...
            for(let i = 0; red_players.length > i; i++) {
                ...
                buildText({ctx: ctx, text: (red_players[i].stats.kills / red_players[i].stats.deaths).toFixed(2), size: 60, x: x_red_kd, y: 2025, color: "#fff", align: "left"})
            ...
            for(let i = 0; blue_players.length > i; i++) {
                ...
                buildText({ctx: ctx, text: (blue_players[i].stats.kills / blue_players[i].stats.deaths).toFixed(2), size: 60, x: x_blue_kd, y: 815, color: "#fff", align: "left"})
...
export const buildMMRImage = async function ({mmrdata, bgcanvas, seasonid} = {}) {
    ...
    buildText({ctx: ctx, text: seasonkey.toUpperCase(), size: 200, x: (multiplier.x + (((125 * multiplier.triangle) / 2) * 8.25)), y: 325, color: color, align: "center", font: "anton"})
    buildText({ctx: ctx, text: seasonvalue[0].patched_tier.toUpperCase(), size: 200, x: (multiplier.x + (((125 * multiplier.triangle) / 2) * 8.25)), y: 2025, color: color, align: "center", font: "anton"})
    ...
    buildText({ctx: ctx, text: `${((entries[1].wins / entries[1].number_of_games) * 100).toFixed(2)}%`, size: 110, x: 3700, y: 1037.5, color: color, align: "right", font: "anton"})
    ...
    for(let i = 0; rowcount > i; i++) {
        ...
        for(let k = 0; tiers.length > k; k++) {
            ...
            ctx.drawImage(triangleimage, (k * x) + ((rowcount - i) * x) + (multiplier.x + (((125 * multiplier.triangle) / 2) * (7 - rowcount))), (i * ((111 * multiplier.triangle) + 2.75)) + (multiplier.y + 400), 125 * multiplier.triangle, 111 * multiplier.triangle)
        }
    }
...
export const errorhandler = async function (error) {
    ...
    return error.message.reply({embeds: [embedBuilder({title: translations[error.lang].response[error.status].title, desc: translations[error.lang].response[error.status][error.type], footer: `VALORANT LABS [ERROR ${error.status}]`})], components: [{type: ComponentType.ActionRow, components: [{type: ComponentType.Button, label: translations[error.lang].support, style: ButtonStyle.Link, url: "https://discord.gg/Zr5eF5D"}]}]})
}
...
export const patchGuild = async function ({interaction, key, value, additionaldata, guilddata} = {}) {
    ...
    switch(key) {
        ...
        case "blacklist": {
            doc = (await getDB("settings").findOneAndUpdate({gid: interaction.guild.id}, {$set: {blacklist: Boolean(value)}}, {upsert: false, returnDocument: "after"})).value
            break
        }
        case "background": {
            if(value == false) {
                doc = (await getDB("settings").findOneAndUpdate({gid: interaction.guild.id}, {$set: {background: value}}, {upsert: false, returnDocument: "before"})).value
                ...
                return interaction.editReply({
                    ...
                    components: [{
                        ...
                        components: [
                            {type: ComponentType.Button, style: ButtonStyle.Success, label: translations[doc.lang].settings.imggenerated_label_accept, customId: `settings;background;${interaction.options.get("type").value};accept`},
                            {type: ComponentType.Button, style: ButtonStyle.Danger, label: translations[doc.lang].settings.imggenerated_label_deny, customId: `settings;background;${interaction.options.get("type").value};deny`}
                        ]
                    }]
                })
            }
        ...
        case "_background": {
            const background = await axios.get(interaction.message.embeds[0].footer.text, {responseType: "arraybuffer"}).catch(error => {return error})
            ...
            if(additionaldata == "stats") doc = (await getDB("settings").findOneAndUpdate({gid: interaction.guild.id}, {$set: {background_stats: value}}, {upsert: false, returnDocument: "after"})).value
            if(additionaldata == "game") doc = (await getDB("settings").findOneAndUpdate({gid: interaction.guild.id}, {$set: {background_game: value}}, {upsert: false, returnDocument: "after"})).value
            if(additionaldata == "mmr") doc = (await getDB("settings").findOneAndUpdate({gid: interaction.guild.id}, {$set: {background_mmr: value}}, {upsert: false, returnDocument: "after"})).value
            break
        }
        case "autoroles": {
            ...
            doc = guilddata.autoroles?.some(item => item.name == value)
                ...
                : (await getDB("settings").findOneAndUpdate({gid: interaction.guild.id}, {$push: {autoroles: {id: additionaldata, name: value}}}, {upsert: false, returnDocument: "after"})).value
