...
module.exports = async function check() {  
  // 1. Load API and Cache
    ...
    let last = fs.readFileSync('./autonews/autonews-de-status-incidents.txt', 'utf8') || 0
    last = parseInt(last)
    ...
    if (article.updates[0].updated_at > last) {
        // 5. Update Cache
        fs.writeFileSync('./autonews/autonews-de-status-incidents.txt', article.updates[0].updated_at)
