const { errorhandlerinteraction, checkPrivacyInteraction, axios, Canvas, agentid, basedata, bot, randomize, text, ranks, moment, maps, modes, agents, MessageAttachment, fs } = require("../../functions.js")
module.exports.execute = async (interaction, args, guilddata) => {
    await interaction.defer()
    var msg;
    var privacycheck = await checkPrivacyInteraction(args[1])
    if(privacycheck.status != 200) return errorhandlerinteraction(interaction, privacycheck.status, "stats", guilddata, privacycheck)
    var matchlist = await axios.get(`https://${privacycheck.region}.api.riotgames.com/val/match/v1/matchlists/by-puuid/${privacycheck.puuid}`, {headers: {"X-Riot-Token": basedata.riottoken}}).catch(error => {return error})
    if(matchlist.response && matchlist.response.status == 429) {
        msg = interaction.editReply({embeds: [{title: "Hit Rate Limit"}]})
        setTimeout(() =>{}, Number(matchlist.response.headers['Retry-After']))
    ...
    if(missing_matches.length) {
        msg = await interaction.message.edit({embeds: [{title: `Fetching ${missing_matches.length} matches`}]})
        for await (matchid of missing_matches) {
            ...
            if(privacycheck.type == "unofficial") {
                if(cmatch.data.data.metadata.mode != "Deathmatch") {
                    ...
                    agent_array.playtime = Number(agent_array.playtime) + cmatch.data.data.game_length
                ...
                if(cmatch.data.matchInfo.queueId != "deathmatch") {
                    ...
                    if(dbindex != -1) userstats.agents.splice(dbindex, 1)
                    var agent_array = agent != undefined ? agent : {agent: "", playtime: 0, matches: 0, kills: 0, deaths: 0, assists: 0, headshots: 0, wins: 0, firstbloods: 0, aces: 0, clutches: 0, flawless: 0}
                    agent_array.playtime = Number(agent_array.playtime) + cmatch.data.matchInfo.gameLengthMillis
                    agent_array.matches = Number(agent_array.matches) + 1
                    agent_array.kills = Number(agent_array.kills) + Number(player.stats.kills)
                    agent_array.deaths = Number(agent_array.deaths) + Number(player.stats.deaths)
                    agent_array.assists = Number(agent_array.assists) + Number(player.stats.assists)
    ...
    text(ctx, `STATS: ${privacycheck.name}`, 130, canvas.width/2, 200, '#ffffff', 'center', "anton", true)
    text(ctx, `${privacycheck.name}#${privacycheck.tag}`, 80, 975, 340, "#ffffff", "center")
    ...
    text(ctx, rank != null ? rank.data.data.elo : "N.A", 120, 750, 925, '#00ffcc')
    ...
    interaction.message.edit({files: [attachment], embeds: [], components: []})
