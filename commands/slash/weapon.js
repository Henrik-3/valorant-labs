...
export async function execute({interaction, guilddata} = {}) {
    const request = await axios.get(`https://valorant-api.com/v1/weapons?language=${translations[guilddata.lang].valorant_api_lang}`).catch(error => {return error})
    ...
    const fields = [
        ...
        {name: translations[guilddata.lang].weapon.firerate_zoom, value: cweapon.weaponStats.adsStats != undefined ? String(cweapon.weaponStats.adsStats.fireRate) : "N.A", inline: true},
        {name: translations[guilddata.lang].weapon.zoom, value: cweapon.weaponStats.adsStats != undefined ? String(cweapon.weaponStats.adsStats.zoomMultiplier) : "N.A", inline: true},
    ]
    for(let i = 0; cweapon.weaponStats.damageRanges > i; i++) {
        fields.push({
            name: `${String(range.rangeStartMeters)}m-${String(range.rangeEndMeters)}`, value: `${translations[guilddata.lang].weapon.head}: ${cweapon.weaponStats.damageRanges[i].headDamage} | ${translations[guilddata.lang].weapon.body}: ${cweapon.weaponStats.damageRanges[i].bodyDamage} | ${translations[guilddata.lang].weapon.leg}: ${cweapon.weaponStats.damageRanges[i].legDamage}`
        })
    }
