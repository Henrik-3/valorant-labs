...
export async function execute(data) {
    ...
    switch(data.interaction.options._subcommand) {
        ...
        case "remove": {
            ...
            return data.interaction.editReply({embeds: [{title: Utils.translations[data.guilddata.lang].blacklist.blacklistremoved_title, description: eval('`' + Utils.translations[data.guilddata.lang].blacklist.blacklistchannels + '`'), color: 0xff4654, timestamp: new Date().toISOString(), footer: {text: 'VALORANT LABS [SETTINGS]', icon_url: "https://valorantlabs.xyz/css/valorant-logo.png"}}]})
        }
    }
}
