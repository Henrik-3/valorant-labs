...
export async function execute({interaction, guilddata} = {}) {
    ...
    if(!dbcheck) {
        ...
        if(topggusage && topggusage.current >= 55) return interaction.editReply({embeds: [embedBuilder({title: translations[guilddata.lang].mmr["429_title"], desc: translations[guilddata.lang].mmr["429_desc"], footer: 'VALORANT LABS [MMR TOP.GG ERROR]'})]})
        const topggvote = await axios.get(`https://top.gg/api/bots/702201518329430117/check?userId=${interaction.user.id}`, {headers: {"Authorization": topgg}}).catch(error => {return error})
        if(topggvote.response) return errorhandlerinteraction({interaction, status: topggvote.response.status, type: "mmr", lang: guilddata.lang})
        await getDB("rate-limit-key").updateOne({key: "topgg"}, {$inc: {current: 1}})
        if(topggvote.response) return interaction.editReply({embeds: [embedBuilder({title: translations[guilddata.lang].mmr["500_title"], desc: translations[guilddata.lang].mmr["500_desc"], footer: 'VALORANT LABS [MMR TOP.GG ERROR]'})]})
    ...
    interaction.editReply({
        files: [attachment], components: [{
            type: ComponentType.ActionRow, components: [{type: ComponentType.SelectMenu, customId: `mmr;${puuid.data.data.region};${puuid.data.data.puuid}`, maxValues: 1, minValues: 0, options: seasonscomponents, placeholder: translations[guilddata.lang].mmr.season_select}]
        }]
    })
}
