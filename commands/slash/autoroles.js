...
export async function execute({interaction, guilddata} = {}) {
    switch(interaction.options._subcommand) {
        case "setup": {
            return patchGuild({interaction, key: "autoroles", value: interaction.options.get("rank").value, additionaldata: interaction.options.get("role").value, guilddata})
        }
        case "get": {
                        ...
                        embedBuilder({
                            ...
                            additionalFields: roles.map(item => {
                                return {
                                    ...
                                    value: guilddata.autoroles.some(item1 => item1.name == item) ? translations[guilddata.lang].autorole.correct : translations[guilddata.lang].autorole.wrong
                                }
                            }),
