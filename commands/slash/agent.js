...
export async function execute({interaction, guilddata} = {}) {
    const request = await axios.get(`https://valorant-api.com/v1/agents?language=${translations[guilddata.lang].valorant_api_lang}`).catch(error => {return error})
    ...
    if(cagent == undefined) return interaction.editReply({embeds: [embedBuilder({title: translations[guilddata.lang].agent.agentunknown_title, desc: translations[guilddata.lang].agent.agentunknown_desc, footer: `VALORANT LABS [AGENT ERROR]`})], components: [{type: ComponentType.ActionRow, components: [{type: ComponentType.Button, label: translations[guilddata.lang].support, style: ButtonStyle.Link, url: "https://discord.gg/Zr5eF5D"}]}]})
    ...
    return interaction.editReply({
        embeds: [embedBuilder({
            ...
            additionalFields: [
                {name: translations[guilddata.lang].agent.agentrole, value: `${agents.find(item => item.name.toLowerCase() == cagent.displayName.toLowerCase()).discord_id} ${cagent.role.displayName}`},
