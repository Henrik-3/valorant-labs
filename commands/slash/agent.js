...
export async function execute(data) {
    ...
    if(cagent == undefined) return data.interaction.editReply({embeds: [{title: Utils.translations[data.guilddata.lang].agent.agentunknown_title, description: Utils.translations[data.guilddata.lang].agent.agentunknown_desc, color: 0xff4654, timestamp: new Date().toISOString(), footer: {text: `VALORANT LABS [AGENT ERROR]`, icon_url: "https://valorantlabs.xyz/css/valorant-logo.png"}}], components: [{type: "ACTION_ROW", components: [{type: "BUTTON", label: Utils.translations[data.guilddata.lang].support, style: "LINK", url: "https://discord.gg/Zr5eF5D"}]}]})
