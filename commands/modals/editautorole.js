	...
	interaction.client.channels.cache.get(args[1]).messages.edit(args[2], {
		embeds: [
			embedBuilder({
				...
				color: interaction.fields.fields.has('color')
					? interaction.fields.getTextInputValue('color').includes('#')
						? parseInt(interaction.fields.getTextInputValue('color').replace('#', ''), 16)
